...
resource "tls_private_key" "catalyst_deploy_key" {
...
resource "local_file" "private_key" {
  count           = var.key_name == null ? 1 : 0
  content         = tls_private_key.catalyst_deploy_key[0].private_key_pem
...
resource "aviatrix_transit_external_device_conn" "pubConns" {
  ...
  bgp_remote_as_num        = var.catalyst_bgp_as_num
  backup_bgp_remote_as_num = local.is_ha ? var.catalyst_bgp_as_num : null
  ha_enabled               = local.is_ha
  direct_connect           = false
  remote_gateway_ip        = aws_eip.catalyst_public_eip[0].public_ip
  backup_remote_gateway_ip = local.is_ha ? aws_eip.catalyst_public_eip[1].public_ip : null
...
resource "aviatrix_transit_external_device_conn" "privConns" {
  ...
  remote_gateway_ip        = var.tunnel_proto == "LAN" ? null : tolist(aws_network_interface.catalyst_Public_ENI[0].private_ips)[0]
  backup_remote_gateway_ip = local.is_ha && var.tunnel_proto != "LAN" ? tolist(aws_network_interface.catalyst_Public_ENI[1].private_ips)[0] : null
  ...
  remote_lan_ip            = var.tunnel_proto == "LAN" ? tolist(aws_network_interface.catalyst_BGPOLAN_ENI[0].private_ips)[0] : null
  backup_remote_lan_ip     = local.is_ha && var.tunnel_proto == "LAN" ? tolist(aws_network_interface.catalyst_BGPOLAN_ENI[1].private_ips)[0] : null
